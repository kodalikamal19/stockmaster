      ...
      {viewMode === 'table' ? (
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data?.items?.map((delivery) => (
                <tr key={delivery.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap font-medium">{delivery.reference}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{delivery.from_location?.name || '-'}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{delivery.to_location?.name || '-'}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{delivery.contact_name || '-'}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{delivery.schedule_date || '-'}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                    ...
                    {delivery.status !== 'validated' && delivery.status !== 'cancelled' && (
                      <>
                        <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {['pending', 'scheduled', 'validated', 'cancelled'].map((status) => {
            const items = data?.items?.filter((d) => d.status === status) || []
            return (
              <div key={status} className="bg-gray-100 rounded-lg p-4">
                <h3 className="font-semibold mb-4 capitalize">{status}</h3>
                <div className="space-y-2">
                  {items.map((delivery) => (
                    <div key={delivery.id} className="bg-white p-3 rounded shadow">
                      <p className="font-medium text-sm">{delivery.reference}</p>
                      <p className="text-xs text-gray-600">{delivery.contact_name || 'No contact'}</p>
                    </div>
                  ))}
                </div>
              </div>
